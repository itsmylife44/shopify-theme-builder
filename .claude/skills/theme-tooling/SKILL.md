...
The Studio composes only the home, product and collection pages. Every other JSON template (page, blog, article, search, 404, password, collections list) uses the Base Theme's own section, a basic layout built from the `.basic-page` classes in `base-theme/assets/critical.css` with a color scheme setting, so it follows the Brand. The contact page is the exception: `catalog/templates/page.contact.json` shows the Base Theme's `page` section above the catalog's `contact-form` (enabled only on `page` templates), and the product skill copies both into every Theme in step 3. So is the cart page: `catalog/templates/cart.json` uses the catalog's `main-cart` (enabled only on `cart` templates, with accelerated checkout buttons, an optional order note and discounts), also copied in step 3. There are no `customers/*` templates: the header links to Shopify's new customer accounts, which need none, so a shop still on classic customer accounts gets no account pages.
