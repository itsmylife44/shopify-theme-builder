...
- **Page sections.** A section that needs a page's object, like `product` for the main product and related products, sets `"enabled_on": {"templates": ["product"]}`, so the Studio offers it only for that page. A section a page shows once, like the main product, sets `"limit": 1`; the Studio refuses to add more than the limit.
