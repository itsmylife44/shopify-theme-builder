...
- **Per-section color scheme.** The schema has `{"type": "color_scheme", "id": "color_scheme", "label": "Color scheme", "default": "scheme-1"}` and the outer element carries `class="color-{{ section.settings.color_scheme }}"`. Both halves depend on the Brand work (issue #5), which adds a `color_scheme_group` to `base-theme/config/settings_schema.json` and the CSS behind the `color-*` classes. Until then, Theme Check passes such a section but a Shopify upload rejects it.
