...
- **Group sections.** A section that belongs in a section group, like `header` and `footer`, sets `"enabled_on": {"groups": ["header"]}` (or `["footer"]`); the Studio leaves it out of the sections it adds to pages. The group file places it (`sections/header-group.json`, `sections/footer-group.json`); when the Base Theme's group file sets settings or blocks the catalog section doesn't have, ship a replacement group file next to it in `catalog/sections/` (the footer does).
- **Storefront text.** Text the Creator writes is a section setting with a default (headings, button labels). Fixed text like accessible labels and form messages uses `{{ 'key' | t }}` with its key added to `base-theme/locales/en.default.json`.
