...
| `…/scripts/` | Scripts the agent runs on a Creator's Theme, like `check-direction.mjs`, the Direction checker run as each Direction is written and at the review before hand-off, `screenshot.mjs`, which captures the preview's pages for that review, and `check-a11y.mjs`, which runs axe-core and a keyboard pass through the drawers and quick add on them |
...
| `test/` | Vitest tests for the Studio, the `create-theme` command, the Direction checker, the screenshot script's arguments, parts and Chrome lookup, the accessibility check's arguments, findings and keyboard pass, the Section Catalog, the skill-gate hook and the release version |
