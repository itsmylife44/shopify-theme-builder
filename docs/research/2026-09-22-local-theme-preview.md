...
- **What the code actually does** (Shopify/cli `packages/theme/src/cli/utilities/theme-environment/`, [S9]):
  ...
  - `theme-fs.ts`: `chokidar` (`3.6.0`) watches the theme directory. **Any process that writes files to that directory, including our editor, triggers upload and hot reload.** We read this from the code but haven't tested it with an external writer. (Update for #7: CLI 4.8.0 still watches with chokidar, `startWatcher` on `add`/`change`/`unlink`; chokidar reports OS file events whatever process writes. Observed live on 2026-09-22 against a development store: a file written by another process synced within a second and rendered.)
