...
2. Images and videos are picked in Shopify's Theme Editor: the selected section in the Studio shows each image and video setting, whether it's set (with a set image's file name), and a "Choose in the Theme Editor" link that opens the page in the Theme Editor on the preview's development theme, where they select that section. Products and menus are edited in the Shopify admin. For a contact page, the Merchant picks the `contact` template for their Contact page in Shopify admin › Online Store › Pages.
