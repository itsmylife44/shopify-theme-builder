...
`<skill-dir>` below is the folder holding this `SKILL.md`. It holds `base-theme/`, `catalog/sections/`, `studio/`, `scripts/` (the Direction checker and the screenshot script) and `references/` (the design method, read when a step names a file). The Theme lives in its own folder outside it; the Theme's files never go in `<skill-dir>`.
...
Once you have the store address, take care of the three things below in one round, so the Creator never meets them later as an interruption or a surprise. For a development store you created, ask right after `store create dev`: a new store starts in English and US dollars, with a demo Home, Catalog, Contact menu, and the preview shows that until they are set.
...
3. **Store decisions**, all in the same message, each with a default the Creator mostly confirms. Take the defaults from what the Creator told you so far and from their website when they named one (its language, its prices, its menus):
   ...
   - **Currency**: the store's currency, like `EUR`.
...
4. Tell the Creator right then, with the links, the three settings the Admin API can't change, and ask them to do them now:
   - the default language, when it isn't the store's primary one: at `https://admin.shopify.com/store/<shop>/settings/languages`, Change default;
   - the currency: at `https://admin.shopify.com/store/<shop>/settings/general`, Store defaults › Currency display (a store with orders may not change it; then its Markets set the prices);
   - the markets: at `https://admin.shopify.com/store/<shop>/settings/markets`, the countries it sells to, with their currency.
...
5. Compose the other pages. A page keeps at least one section, so add the new sections before removing the Base Theme's `main`:
   - **product**: add `main-product` and `related-products`, then remove `main`. Set the main product's `gallery_layout` (grid, stacked, thumbnails or carousel) to suit the Direction. Leave its `image_zoom` on `lightbox` (the default: a product image opens full screen, with pinch to zoom on touch); `none` only when the Creator asks.
...
5. The store setup from step 1, confirmed, not discovered: the main and footer menus you wrote and the languages you enabled and published. Check what only the Creator could set with `{ shop { currencyCode } shopLocales { locale primary published } }` through `store execute`: when the default language isn't `primary` or the currency isn't the agreed one, say so, with its admin link from step 1 again. Once the default language is `primary`, unpublish a language the shop doesn't sell in (a new store's English) with `shopLocaleUpdate` and `{published: false}`, so the footer offers no language the page text isn't in. Ask whether the markets are set, and list each menu item left out for a missing page or policy.
...
2. Follow the conventions of the catalog sections; open one in `<skill-dir>/catalog/sections/` (like `image-with-text.liquid`) as the model:
   - **Brand only through settings.** No hardcoded colors or fonts: use the CSS variables the Theme sets from the Brand (`--color-background`, `--color-foreground`, `--color-button`, `--color-button-label`, `--font-heading--*`, `--font-body--*`, `--font-accent--*`); a price takes the class `price`, which gives it the accent font and tabular figures, and a sale price sits in a `price__sale` span, in the accent color; a section that stands in for the product page's price (its `_product-price` block) keeps `<p class="rte">{%- render 'tax-note' -%}</p>` under the price, the Theme's snippet saying whether taxes and duties are included, with shipping linked to the shipping policy (the Theme Store requires it). No font sizes or line heights either: a heading takes its size from its level, and a class `text-display`, `text-h1` to `text-h6`, `text-body`, `text-small` or `text-label` gives any element another step of the Theme's type scale. Spacing, widths and borders come from the Theme's variables too: `--space-2xs` to `--space-2xl` for padding, margins and gaps, `--section-spacing` above and below the section, `--grid-gap` between the items of a grid (`--grid-row-gap` between rows of cards with text), which follow the Theme's density setting, `--width-narrow`, `--width-text` or `--width-prose` for a column's width, `min-inline-size` and `min-block-size` of `var(--target-size)` (44px) on an icon button or option picker and `var(--target-size-min)` (24px) on any other small control, `var(--border-width) solid var(--color-border)` for a border, `var(--style-border-radius-cards)`, `var(--style-border-radius-media)`, `var(--style-border-radius-inputs)` or `var(--style-border-radius-badges)` for corners, which follow the Theme's shape setting (never a radius of their own), and for a product or collection image `background-color: var(--media-background)` on its box with `padding: var(--media-inset)`, `object-fit: var(--media-fit)` and `mix-blend-mode: var(--media-blend)` on the image, which follow the Theme's media settings (any other image always covers its box with `object-fit: cover`), `var(--opacity-muted)` for muted text. The focus ring, the `visually-hidden` class and placeholder colors (with the `placeholder` class) are already shared, and layouts switch at `@media (min-width: 750px)`. Buttons, and links styled as buttons, take the shared `button` or `button--secondary` class and no colors, padding or shape of their own. No entrance animations: the Theme already fades sections in on scroll, following its motion setting; a hover transition takes `var(--motion-duration) var(--motion-easing)` inside `@media (prefers-reduced-motion: no-preference)`.
