...
| **Sizing** | For clothing, shoes or anything sized: how it fits, and the shop page with its size chart, if there is one | The `size-guide` block after the variant picker, which opens the size chart page in a dialog, and how it fits in a collapsible block on the product page, when it holds for every product. Fit that differs per product comes from a product metafield the Creator connects (the hand-off) |
...
The product page's text shows on every product: write only facts that hold for the whole shop there. Facts that differ per product (materials, fit, care) come from a product metafield the Creator connects in the Theme Editor, never from one product's facts written into the template.
...
Before writing the Directions, read the brief back to the Creator in a few lines and have them confirm it: the six items as you understood them, the seven things from the brand's world, the category default and its opposite you will steer clear of, and what each reference lends. Ask it as one question with the brief shown in it (like Claude Code's `AskUserQuestion` with the brief in an option's preview), so the Creator can confirm or correct it in one answer. A question with previews has no **Other** option: tell the Creator to correct a line with a note on the option (press `n`), or add a "Change something" option. Under pinned, list only what the Creator gave or you read from their own site, and name the logo only when one was found or given; otherwise say "no logo".
