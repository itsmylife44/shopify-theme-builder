...
Give each color a name and a job ("press-cloth linen: page background"), 4 to 6 in all, and map them onto schemes: `scheme-1` the main one, `scheme-2` its inverse, `scheme-3` the accent's section. A Committed or Full Direction puts at least one section of its home on `scheme-3`: that's the region the color owns. A Direction may use more schemes than the others, like a `scheme-4`: Shopify wants the same scheme ids in every Direction, so the Studio gives the others that id too, as a copy of their `scheme-1`. Contrast stays at 4.5:1 for text and 3:1 for borders in every Direction. Choose light or dark from the shopper's scene, never from the category.
