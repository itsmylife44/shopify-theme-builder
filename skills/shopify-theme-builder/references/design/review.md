...
The script drives the system's Google Chrome (or Chromium, or Microsoft Edge) headless, captures the full page with reduced motion, so every section shows (the reveal on scroll would leave sections below the fold blank), and ends within a minute, 20 seconds more for each other page. With `--parts` it also writes each page top to bottom in parts twice the viewport tall, `home-1440-1.png`, `home-1440-2.png`, …, and prints their paths (`--part-height <px>` sets another height). Don't crop or split the captures yourself. Don't use Chrome's own `--screenshot` flag: it hangs on the preview, whose hot reload never goes idle, and can't lay out 390 pixels wide. It prints each page's `scrollWidth`: when it's wider than the viewport, something overflows sideways, a finding for the floor. Under it, `the header cuts off its content:` names each element of the header too narrow for what it holds, like `a.header__logo shows 160 of its 241px` for a shop name too big for a phone: the page's `scrollWidth` misses it, since the header hides the overflow instead of widening the page. That's a finding for the floor too. On a store whose market is in the EU, Shopify shows its cookie consent banner over the middle of the page to every fresh browser: the script dismisses it before capturing (it accepts the tracking and removes the banner), so the page shows whole. Add `--keep-consent` only to review the banner itself. When it finds no browser, ask the Creator to install Google Chrome (or set `CHROME_PATH` to one), or use another browser tool you have. Wait until the preview has synced the last write (the Studio's log shows it) before taking them. When nothing can take a screenshot, say so in the verdict: the review then rests on the checker and the files alone.
...
It runs axe-core's WCAG 2.2 A and AA rules on the page, then a keyboard pass through the menu drawer, the cart drawer and quick add, each where the page shows it: Tab reaches the control, Enter opens its dialog with focus inside, Tab keeps focus in the dialog, Escape closes it, and focus returns to the control ([the APG modal dialog pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/)). Both run without Shopify's cookie consent banner, so the Tab pass reaches the header's drawers; when the page shows the banner, axe also runs once with it first, since it is what a first-time visitor meets. It prints one line per finding, `axe <rule> (<impact>): <what>: <elements>` or `keyboard <control>: <what failed>`, with `with the consent banner` after the impact for what only that run found, and which controls it pressed, and exits 1 when there is any. A finding with the consent banner in Shopify's own banner isn't the Theme's: tell the Creator, whose Shopify admin sets the banner's look under Settings > Customer privacy. Each finding breaks the floor: fix it in the Theme (a Brand color that fails contrast with `PUT /api/brand`, after telling the Creator), and check that page again. It uses the same browser as the screenshots and ends within 90 seconds.
