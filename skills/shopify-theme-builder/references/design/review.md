...
| `movement` | A home page, the Theme's or a Direction's in `listings/<name>/templates/index.json`, where nothing moves or responds: no slideshow, marquee, testimonials or collection list carousel, or product cards with the second image on hover (`card_hover`) | Add the moving element the Direction's card names (`directions.md`, Movement), or set `card_hover` to `second_image` (`PUT /api/style`) |
