#!/usr/bin/env node
// Usage: wait-for-choice [--port 5173] [--timeout 3600]
// Waits until the Creator presses Choose in the Studio on <port>: polls GET /api/theme every 2 seconds and prints the
...
const { values } = parseArgs({ options: { port: { type: 'string', default: '5173' }, timeout: { type: 'string', default: '3600' } } })
