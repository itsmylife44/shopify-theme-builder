...
const styleGroups = {
  Type: ['type_body_size', 'type_scale_ratio', 'type_display_size', 'type_display_size_mobile', 'type_heading_weight', 'type_heading_case', 'type_heading_tracking', 'type_heading_line_height'],
...
/**
 * Writes style settings, the social media links (social_instagram …) the footer's social block shows, and the
 * free-shipping threshold the cart and shipping note show, into config/settings_data.json, checked against their
 * schema. An empty color or link, or a null threshold, clears it.
 * @param {string} theme
...
function setStyle(theme, change) {
  ...
  const global = readGlobalSchema(theme)
  const social = Object.values(global).filter((setting) => setting.id.startsWith('social_') && setting.type === 'url')
  ...
  const settings = [...readStyleSchema(theme).flatMap((group) => group.settings), ...social, ...(global.free_shipping_threshold ? [global.free_shipping_threshold] : [])]
...
// (SKILL.md step 1): the menus, the languages, the sample products and their publishing. One command grants them all, so authenticating again for the Studio
