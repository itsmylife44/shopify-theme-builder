...
describe('skill gate hook', () => {
  it('blocks a Liquid edit when the skill was not loaded, naming the skill', () => {
    const result = edit(randomUUID(), 'skills/shopify-theme-builder/catalog/sections/hero.liquid')
    expect(result.code).toBe(2)
    expect(result.stderr).toContain('shopify-liquid')
  })

  it('blocks Write as well as Edit', () => {
    expect(edit(randomUUID(), 'skills/shopify-theme-builder/catalog/sections/hero.liquid', 'Write').code).toBe(2)
  })
  ...
  it('allows a Liquid edit once both Liquid skills were loaded this session', () => {
    ...
    expect(edit(session, 'skills/shopify-theme-builder/catalog/sections/hero.liquid').code).toBe(0)
  })

  it('blocks a Liquid edit until theme-tooling is loaded too, naming it', () => {
    const session = randomUUID()
    loadSkill(session, 'shopify-liquid')
    const blocked = edit(session, 'skills/shopify-theme-builder/catalog/sections/hero.liquid')
  ...
  it('does not carry loaded skills over to another session', () => {
    ...
    expect(edit(randomUUID(), 'skills/shopify-theme-builder/catalog/sections/hero.liquid').code).toBe(2)
  })
  ...
  it('blocks Studio UI code until its skill is loaded', () => {
    const session = randomUUID()
    const blocked = edit(session, 'skills/shopify-theme-builder/studio/src/App.tsx')
    expect(blocked.code).toBe(2)
    expect(blocked.stderr).toContain('vercel-react-best-practices')

    loadSkill(session, 'vercel-react-best-practices')
    expect(edit(session, 'skills/shopify-theme-builder/studio/src/App.tsx').code).toBe(0)
  })
...
describe('dev skills', () => {
  // `npx skills add` skips a .claude/skills/<name> listed in skills-lock.json, so this keeps the
  // repo's dev skills out of a Creator's install, which gets only skills/shopify-theme-builder.
  it('are all listed in skills-lock.json', () => {
    const locked = Object.keys(JSON.parse(readFileSync(`${projectDir}skills-lock.json`, 'utf8')).skills)
    expect(locked).toEqual(expect.arrayContaining(readdirSync(`${projectDir}.claude/skills`)))
  })
})
